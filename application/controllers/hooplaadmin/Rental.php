...
class Rental extends Admin_Controller{
	...
	public function rentallist($id = NULL){
		...
		foreach ($data['listrental'] as $key => $value) {
			$map = directory_map('assets/upload/rental/pic-rental-'.seo_url($data['listrental'][$key]->namaRENTAL), FALSE, TRUE);
			
			if (empty($map)) {
				$data['listrental'][$key]->imageRENTAL = base_url() . 'assets/upload/no-image-available.png';
			} else {
				$data['listrental'][$key]->imageRENTAL = base_url() . 'assets/upload/rental/pic-rental-'.seo_url($data['listrental'][$key]->namaRENTAL).'/'.$map[0];
			}

			if($value->statusRENTAL == 1){
			...
			$data['listrental'][$key]->status = $status;
		}
		...
		if($id == NULL){
			...
			$map = directory_map('assets/upload/rental/pic-rental-'.seo_url($data['getrental']->namaRENTAL), FALSE, TRUE);
			
			if (empty($map)) {
				$data['getrental']->imageRENTAL = '';
			} else {
				$data['getrental']->imageRENTAL = base_url() . 'assets/upload/rental/pic-rental-'.seo_url($data['getrental']->namaRENTAL).'/'.$map[0];
			}
		}
	...
	public function saverental() {
		...
		if ($this->form_validation->run() == TRUE) {
			$data = $this->Rental_m->array_from_post(array('namaRENTAL','hargaRENTAL','statusRENTAL','durasiRENTAL','descriptionRENTAL','idCATEGORY'));
			if($data['statusRENTAL'] == 'on')$data['statusRENTAL']=1;
			else $data['statusRENTAL']=0;
			$data['hargaRENTAL'] = str_replace(['Rp.',' ',','], ['','',''], $data['hargaRENTAL']);
			$id = decode(urldecode($this->input->post('idRENTAL')));

			if(empty($id))$id=NULL;
			$subject = $this->input->post('namaRENTAL');
			// $check = check_filename($id, 'namaRENTAL', 'barang_rental','idRENTAL');
			// if($subject != $check->namaRENTAL){
			// 	$path = 'assets/upload/rental/pic-rental-'.seo_url($check->namaRENTAL);
			// }
			$filenamesubject = 'pic-rental-'.seo_url($subject);
	...
	public function deleteimgrental($id1=NULL){
		if($id1 != NULL){
			...
			$rentaldata = $this->Rental_m->selectall_rental($id)->row();
			$map = directory_map('assets/upload/rental/pic-rental-'.seo_url($rentaldata->namaRENTAL), FALSE, TRUE);
			$path = 'assets/upload/rental/pic-rental-'.seo_url($rentaldata->namaRENTAL);
			foreach ($map as $value) {
				unlink('assets/upload/rental/pic-rental-'.seo_url($rentaldata->namaRENTAL).'/'.$value);
			}
			if(is_dir($path)){
				rmdir($path);
			}
